...
class Analyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      int SelectMu(const edm::Handle<reco::MuonCollection>& muons, const reco::VertexCollection::const_iterator& pv, const reco::BeamSpot& beamSpot);
      int SelectEl(const edm::Handle<reco::GsfElectronCollection>& electrons, const reco::VertexCollection::const_iterator& pv, const edm::Handle<reco::MuonCollection>& muons, const reco::BeamSpot& beamSpot);
      int SelectJet(const edm::Handle<reco::PFJetCollection>& jets, const reco::JetTagCollection& bTags, const edm::Event& iEvent, const edm::EventSetup& iSetup, const edm::Handle<reco::MuonCollection>& muons, const reco::VertexCollection::const_iterator& pv, const edm::Handle<reco::GsfElectronCollection>& electrons, const reco::BeamSpot& beamSpot);
      ...
      float _muTIP[_maxNmu];
      int _muTrackGlobal[_maxNmu];
      int _muPrompt[_maxNmu];
      // electrons
      ...
      float _elDeltaR[_maxNel][_maxNmu];
      float _elTIP[_maxNel];
      // jets
...
Analyzer::Analyzer(const edm::ParameterSet& iConfig)
{
  ...
  if(_flagRECO)
  {
    ...
    _tree->Branch("muTIP",_muTIP,"muTIP[Nmu]/F"); // muon transversal impact parameter
    _tree->Branch("muTrackGlobal",_muTrackGlobal,"muTrackGlobal[Nmu]/I"); //muon tag if are global and tracker muon
    _tree->Branch("muPrompt",_muPrompt,"muPrompt[Nmu]/I"); //muon tag if are globalMuonPromptTight
    // electrons
    ...
    _tree->Branch("elDistPV0",_elDistPV0, "elDistPV0[Nel]/F"); // electron distance to the primary vertex (projection on transverse plane)
    _tree->Branch("elDistPVz",_elDistPVz, "elDistPVz[Nel]/F"); // electron distance to the primary vertex (z projection) 
    _tree->Branch("elDeltaR",_elDeltaR, "elDeltaR[Nel][Nmu]/F"); // electron and muon DeltaR
    _tree->Branch("elTIP",_elTIP, "elTIP[Nel]/F"); //electron transversal impact parameter
    ...
    // mumu triggers
      _vecTriggerNames.push_back("HLT_DoubleMu7_v");
      _vecTriggerNames.push_back("HLT_Mu13_Mu8_v");
       _vecTriggerNames.push_back("HLT_Mu17_Mu8_v");
                    _vecTriggerNames.push_back("HLT_Mu17_TkMu8_v");
                        _vecTriggerNames.push_back("HLT_DoubleMu5_v");
                            // ee triggers
                                _vecTriggerNames.push_back("HLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL_v");
                                    _vecTriggerNames.push_back("HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v");
                                        _vecTriggerNames.push_back("HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v");
                                            // emu triggers
                                                _vecTriggerNames.push_back("HLT_Mu8_Ele17_CaloIdL_v");
                                                    _vecTriggerNames.push_back("HLT_Mu8_Ele17_CaloIdT_CaloIsoVL_v");
                                                        _vecTriggerNames.push_back("HLT_Mu17_Ele8_CaloIdL_v");
                                                            _vecTriggerNames.push_back("HLT_Mu17_Ele8_CaloIdT_CaloIsoVL_v");
                                                                _vecTriggerNames.push_back("HLT_Mu8_Ele17_CaloIdL_v");
                                                                    _vecTriggerNames.push_back("HLT_Mu10_Ele10_CaloIdL_v");
...
// muon selection
int Analyzer::SelectMu(const edm::Handle<reco::MuonCollection>& muons, const reco::VertexCollection::const_iterator& pv, const reco::BeamSpot& beamSpot)
{
  _Nmu = 0;
  // loop over muons
  for (reco::MuonCollection::const_iterator it = muons->begin(); it != muons->end(); it++)
  {
    ...
    if(TMath::Abs(it->eta()) > 2.4)
      continue;
    // selecionamos muons que são GlobalMuon e TrackerMuon
    if((!it->isTrackerMuon() || !it->isGlobalMuon()))
      continue;
    // selecionamos muons com chi2/ndof menor que 10 para o fit do global muon
//    if((it->globalTrack()->normalizedChi2() >= 10))
//      continue;
    // Selecionamos muons com impacto de parametro transversal (xy) < 0.02 cm aplicado no traker do inner tracker
 

     math::XYZPoint point(beamSpot.x0(),beamSpot.y0(), beamSpot.z0());
    double d0 = -1.*it->innerTrack()->dxy(point);
    if (fabs(d0) >=0.02)
      continue;
//   if(fabs(it->innerTrack()->dxy(pv->position())) >= 0.2)
//      continue;
      
    // Selecionamos apenas os muons que passaram pelo filtro do GlobalMuonPromptTight
    bool isGlobalMuonPromptTight = muon::isGoodMuon(*it,muon::GlobalMuonPromptTight);  
	if(!isGlobalMuonPromptTight) 
      continue;
    if(TMath::Abs(pv->z() - it->globalTrack()->vz()) >= 1)
      continue;   
    // fill isolation variables
    const reco::MuonPFIsolation& iso03 = it->pfIsolationR03();
    _muIso03[_Nmu] = ( iso03.sumChargedParticlePt + iso03.sumNeutralHadronEt + iso03.sumPhotonEt) / it->pt();
    ...
    if(_muIso03[_Nmu] >= 0.20)
      continue;
    // fill number of hits variables
    _muHitsValid[_Nmu] = 0;
    _muHitsPixel[_Nmu] = 0;
    const reco::HitPattern& p = (it->innerTrack())->hitPattern();
    ...
    //printf("Loop muons \n");
    // Selecionamos muons com numero de hits no tracker maior que 10
    if(_muHitsValid[_Nmu] <= 10)
       continue;
    ...
    // fill transversal impact parameter
    _muTIP[_Nmu] = fabs(it->innerTrack()->dxy(pv->position()));
    // fill if are or not globalMuonPromptTight
    if(!isGlobalMuonPromptTight){ _muPrompt[_Nmu]=0;}
    if(isGlobalMuonPromptTight){ _muPrompt[_Nmu]=1;} 
    // fillif are or not global and tracker muon
    if((!it->isTrackerMuon() || !it->isGlobalMuon())){ _muTrackGlobal[_Nmu]=0;}
    if((it->isTrackerMuon() && it->isGlobalMuon())){ _muTrackGlobal[_Nmu]=1;}
    // store muon
    _Nmu++;
...
// electron selection
int Analyzer::SelectEl(const edm::Handle<reco::GsfElectronCollection>& electrons, const reco::VertexCollection::const_iterator& pv, const edm::Handle<reco::MuonCollection>& muons, const reco::BeamSpot& beamSpot)
{
  _Nel = 0;
  // loop over electrons
  for (reco::GsfElectronCollection::const_iterator it = electrons->begin(); it != electrons->end(); it++)
  {
    ...
    if(_elIso03[_Nel] >= 0.17)
      continue;
    // selecionamos eletron com impacto de parametro transversal < 0.04 cm  
 
     math::XYZPoint point(beamSpot.x0(),beamSpot.y0(), beamSpot.z0());
    double d0 = -1.*it->gsfTrack()->dxy(point);
    if (fabs(d0) >=0.04)
      continue;


//   if(fabs(it->gsfTrack()->dxy(pv->position())) > 0.4)
//	  continue;
	if(TMath::Abs(pv->z() - it->vz())>=1)
	  continue;
	// Agora faremos um loop nos muons para que possamos ver se eles tem DeltaR > 0.1 com relação aos eletrons, os eletrons que não tiverem essa diferença seram eliminados.
		bool EleCloseMu = false ;
		Nmu = 0;
		for (reco::MuonCollection::const_iterator it2 = muons->begin(); it2 != muons->end(); it2++){
			if(Nmu == _maxNmu)
			{break;}
			double deltaR=10;
			double MuEta= it2->eta(),MuPhi= it2->phi();
		    double EleEta= it->eta() ,ElePhi= it->phi();
			if((!it2->isTrackerMuon() && !it2->isGlobalMuon()))
			continue;
			 _muHitsValid[_Nmu] = 0;
			 const reco::HitPattern& p = (it2->innerTrack())->hitPattern();
			for (int i = 0; i < p.numberOfHits(); i++) 
			{
				uint32_t hit = p.getHitPattern(i);
				if (p.validHitFilter(hit))
			...
			Nmu++;
			double VarEta = (MuEta - EleEta);
			double VarPhi = (MuPhi - ElePhi);
			deltaR=sqrt(pow(VarEta,2) + pow(VarPhi,2));
		//	std::cout<<"deltaR :"<<deltaR<<std::endl;
            if(deltaR < 0.1){EleCloseMu = true;}
            _elDeltaR[_Nel][Nmu]=deltaR;
		}	
	if(EleCloseMu)
	   continue;
     if(((it->gsfTrack())->trackerExpectedHitsInner()).numberOfHits()>=2 || fabs(it->convDist())<=0.02 || fabs(it->convDcot())<= 0.02)
	continue;
	//printf("Loop eletrons \n");
    // fill three momentum (pT, eta, phi)
    ...
    // Fill transversal impact parameter
    _elTIP[_Nel] = fabs(it->gsfTrack()->dxy(pv->position()));
    // store electron
    _Nel++;
...
// jet selection
int Analyzer::SelectJet(const edm::Handle<reco::PFJetCollection>& jets, const reco::JetTagCollection& bTags, const edm::Event& iEvent, const edm::EventSetup& iSetup,const edm::Handle<reco::MuonCollection>& muons, const reco::VertexCollection::const_iterator& pv, const edm::Handle<reco::GsfElectronCollection>& electrons, const reco::BeamSpot& beamSpot)
{
  _Njet = 0;
  ...
  for (reco::PFJetCollection::const_iterator it = jets->begin(); it != jets->end(); it++)
  {
    ...
    // select jet: pT > 30 GeV, |eta| < 2.5
    if(corjet.pt() < 30)
      continue;
    if(TMath::Abs(corjet.eta()) > 2.5)
      continue;
    // Loose Jet Identification
    reco::Candidate::LorentzVector uncorrJet;
    // take the information of the uncorrJet (just work if we don't use a PAT!!!)
    uncorrJet = it->p4();
	double nhf = corjet.neutralHadronEnergy() / uncorrJet.E();
    double nef = corjet.neutralEmEnergy() / uncorrJet.E();
    double chf = corjet.chargedHadronEnergy() / uncorrJet.E();
    double cef = corjet.chargedEmEnergy() / uncorrJet.E();
    int nconstituents = corjet.numberOfDaughters();
    int nch = corjet.chargedMultiplicity();
	bool goodJet = 
                  nhf < 0.99 &&
                  nef < 0.99 &&
                  chf > 0.00 &&
    ...
    if(!goodJet)
    continue;
    //We need know now if the jet have DeltaR more then 0.4 with another leptons. We will make two loop, first the loop of Muons.
    int JetCloseMu = 1;
    for (reco::MuonCollection::const_iterator it2 = muons->begin(); it2 != muons->end(); it2++)
	{
		// selection: pT > 20 GeV, |eta| < 2.4
		if(it2->pt() < 20)
		continue;
		if(TMath::Abs(it2->eta()) > 2.4)
		continue;
		// selecionamos muons que são GlobalMuon e TrackerMuon
		if((!it2->isTrackerMuon() || !it2->isGlobalMuon()))
		continue;
		// selecionamos muons com chi2/ndof menor que 10 para o fit do global muon
		if((it2->globalTrack()->normalizedChi2() >= 10))
		continue;
		// Selecionamos muons com impacto de parametro transversal (xy) < 0.02 cm aplicado no traker do inner tracker
		//if(fabs(it2->innerTrack()->dxy(pv->position())) >= 0.2)
		//continue;
		     math::XYZPoint point(beamSpot.x0(),beamSpot.y0(), beamSpot.z0());
    double d0 = -1.*it2->innerTrack()->dxy(point);
    if (fabs(d0) >=0.02)
      continue;
		// Selecionamos apenas os muons que passaram pelo filtro do GlobalMuonPromptTight
		bool isGlobalMuonPromptTight = muon::isGoodMuon(*it2,muon::GlobalMuonPromptTight);  
		if(!isGlobalMuonPromptTight) 
		continue;
		if(TMath::Abs(pv->z() - it2->globalTrack()->vz()) >= 1)
		continue;   
		// fill isolation variables
		const reco::MuonPFIsolation& iso03 = it2->pfIsolationR03();
		_muIso03[_Nmu] = ( iso03.sumChargedParticlePt + iso03.sumNeutralHadronEt + iso03.sumPhotonEt) / it2->pt();
		if(_muIso03[_Nmu] >= 0.20)
		continue;
		// fill number of hits variables
		_muHitsValid[_Nmu] = 0;
		...
		if(!it2->isPFMuon())
		      continue;	
		// Now we got the DeltaR of Jet-Muon
			double deltaR=10;
			double MuEta= it2->eta(),MuPhi= it2->phi();
		    double JetEta= corjet.eta() ,JetPhi= corjet.phi();
			double VarEta = (MuEta - JetEta);
			double VarPhi = (MuPhi - JetPhi);
			deltaR=sqrt(pow(VarEta,2) + pow(VarPhi,2));
            if(deltaR < 0.4){JetCloseMu=0;}
	}
	if(JetCloseMu==0)
	continue;
	
	//Now we make the same but now to system Jet-Eletron
	bool JetCloseEle=false;
	for (reco::GsfElectronCollection::const_iterator it3 = electrons->begin(); it3 != electrons->end(); it3++)
  {
    // selection: pT > 20 GeV, |eta| < 2.4
    if(it3->pt() < 20)
      continue;
    if(TMath::Abs(it3->eta()) > 2.5)
      continue;
    // fill isolation
    _elIso03[_Nel] = (it3->dr03TkSumPt() + it3->dr03EcalRecHitSumEt() + it3->dr03HcalTowerSumEt()) / it3->pt();
    if(_elIso03[_Nel] >= 0.17)
      continue;
    // selecionamos eletron com impacto de parametro transversal < 0.04 cm  
    //if(fabs(it3->gsfTrack()->dxy(pv->position())) > 0.4)
//	  continue;
     math::XYZPoint point(beamSpot.x0(),beamSpot.y0(), beamSpot.z0());
    double d0 = -1.*it3->gsfTrack()->dxy(point);
    if (fabs(d0) >=0.04)
      continue;


	  if(TMath::Abs(pv->z() - it3->vz())>=1)
	  continue;
	// Agora faremos um loop nos muons para que possamos ver se eles tem DeltaR > 0.1 com relação aos eletrons, os eletrons que não tiverem essa diferença seram eliminados.
		bool EleCloseMu = false;
		int Nmuo;
		...
		for (reco::MuonCollection::const_iterator it2 = muons->begin(); it2 != muons->end(); it2++){
			double deltaR=10;
			double MuEta= it2->eta(),MuPhi= it2->phi();
		    double EleEta= it3->eta() ,ElePhi= it3->phi();
			if((!it2->isTrackerMuon() && !it2->isGlobalMuon()))
			continue;
			 _muHitsValid[_Nmu] = 0;
			 _muHitsPixel[_Nmu] = 0;
	...
	if(EleCloseMu)
	   continue;
	if(((it3->gsfTrack())->trackerExpectedHitsInner()).numberOfHits()>=2 || fabs(it3->convDist())<=0.02 || fabs(it3->convDcot())<= 0.02)
	continue; 
	//Now we got the DeltaR of Jet-Electron
	  double deltaR=10;
			double EleEta= it3->eta(),ElePhi= it3->phi();
		    double JetEta= corjet.eta() ,JetPhi= corjet.phi();
			double VarEta = (EleEta - JetEta);
			double VarPhi = (ElePhi - JetPhi);
			deltaR=sqrt(pow(VarEta,2) + pow(VarPhi,2));
            if(deltaR < 0.4){JetCloseEle=true;}
	}
	if(JetCloseEle)
	continue;
...
void Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 iEvent.getByLabel("offlineBeamSpot", beamSpotHandle);
   ...
   // muons
    iEvent.getByLabel(_inputTagMuons, muons);
    SelectMu(muons, pv, beamSpot);
    // electrons
    iEvent.getByLabel(_inputTagElectrons, electrons);
    SelectEl(electrons, pv, muons,beamSpot);
    ...
    SelectJet(jets, bTags, iEvent, iSetup, muons ,pv, electrons, beamSpot);
    // require two jets
    if( _Njet >= 0)
      selRECO = true;
