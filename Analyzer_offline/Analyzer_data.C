...
leg12->AddEntry(h12,"data ","f");
leg12->SetBorderSize(0);

TFile *MyFile = new TFile("ht_data.root","recreate");
...
double maxPtDiLep=-1;
int N_ee=0,N_emu=0,N_mumu=0,pvNDOF,evRunNumber;
...
t->SetBranchAddress("evRunNumber",&evRunNumber);
//Primary Vertex
...
//		if(evRunNumber<=180542){
		if(evRunNumber<=167913){
			for(int bit1 = 8; bit1 < 9;bit1++){
				if((Triggers >> bit1) & 1)
				{
				trigemu = true;
				break;
               ...
               if(evRunNumber>=167914){
			for(int bit1 = 9; bit1 < 10;bit1++){
				if((Triggers >> bit1) & 1)
				{
				trigemu = true;
				break;
		...
		if(evRunNumber<=175972){
			for(int bit1 = 10; bit1 < 11;bit1++){
				if((Triggers >> bit1) & 1)
				{
				trigemu = true;
				break;
		...
		if(evRunNumber>=175973){
			for(int bit1 = 11; bit1 < 12;bit1++){
				if((Triggers >> bit1) & 1)
				{
				trigemu = true;
				break;
		...
		if(evRunNumber>=165209 && evRunNumber<=178419){
			for(int bit2 = 1; bit2 < 2;bit2++){
				if((Triggers >> bit2) & 1)
				{
				trigmumu = true;
				break;
		...
		if(evRunNumber<=178419){
			for(int bit2 = 2; bit2 < 4;bit2++){
				if((Triggers >> bit2) & 1)
				{
				trigmumu = true;
				break;
		...
		for(int c=0;c<Nmu;c++){
			...
			jato1.SetPtEtaPhiM(TMath::Abs(jetPt[g]), jetEta[g], jetPhi[g], jetMass[g]);
				...
				if(jetBTagDiscr[g] > 0.244){JetB=true;NJetB++;}
					...
					if(jatinhosE && mumu_1b){h6->Fill(MET,peso);h8->Fill(mass_inva,peso);}
					if(jatinhosE && mumu_1b && MET>=40){h10->Fill(pt_muon,peso);h12->Fill(pt_jato,peso);}
					if(MET>=40 && mumu_1b && mumu_0b){h1->Fill(NJetB,peso);}
					if(MET>=40 && mumu_0b){h4->Fill(Njet,peso);}
  ...
  if(emu){
		N_emu++;
			...
			for(int e=0;e<Nmu;e++){
				...
				if(jatinhosE && emu_0b){h7->Fill(MET,peso);h9->Fill(mass_inva,peso);h12->Fill(pt_jato,peso);}
				if(emu_0b){h1->Fill(NJetB,peso);h5->Fill(Njet,peso);}
			//	if(Njet<=1){continue;}h3
			//	h1->Fill(NJetB,peso);
		...
		if(ee){
				...
				//	if(!ee_1b){continue;}
					...
					if(jatinhosE && ee_1b){h6->Fill(MET,peso);h8->Fill(mass_inva,peso);}
					if(jatinhosE && ee_1b && MET>=40){h11->Fill(pt_electron,peso);h12->Fill(pt_jato,peso);}
					if(MET>=40 && ee_1b && ee_0b){h1->Fill(NJetB,peso);}
					if(MET>=40 && ee_1b){h4->Fill(Njet,peso);}
				//	if(Njet<=1){continue;}
