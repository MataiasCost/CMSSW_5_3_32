...
leg2->AddEntry(h2,"t#bar{t} signal","f");
...
leg3->AddEntry(h3,"t#bar{t} signal","f");
...
leg4->AddEntry(h4,"t#bar{t} signal","f");
...
leg5->AddEntry(h5,"t#bar{t} signal","f");
...
leg6->AddEntry(h6,"t#bar{t} signal","f");
...
leg7->AddEntry(h7,"t#bar{t} signal","f");
...
leg8->AddEntry(h8,"t#bar{t} signal","f");
...
leg9->AddEntry(h9,"t#bar{t} signal","f");
...
leg10->AddEntry(h10,"t#bar{t} signal","f");
...
leg11->AddEntry(h11,"t#bar{t} signal","f");
...
leg12->AddEntry(h12,"t#bar{t} signal","f");
leg12->SetBorderSize(0);

TFile *MyFile = new TFile("ht_ttbar.root","recreate");
...
double peso = 0.00685933518421424751565499595277;
float MET,pvRho,pvZ;
...
ee_5=0;
	...
	for(Int_t i=0;i<n;i++)
    {
		...
		if(trigee){
			//LOOP do canal ee
			for(int a=0;a<Nel;a++){
//			if(elMissHits[a]>=2 || elConvDist[a]<=0.02 || elConvDcot[a]<= 0.02){continue;}
		...
		//LOOP dos TRIGGERS do canal emu5 7th~11th      12 to 14
	...
	if(mumu){
		...
		//	if(Njet<=1){continue;}
					...
					if(MET>=40 && mumu_2b && mumu_1b && mumu_0b){mumu_3b=true;}
  ...
  if(emu){
		...
		if(ee){
				...
				//	if(!ee_1b){continue;}
					...
					if(MET>=40 && ee_0b){h4->Fill(Njet,peso);}
				//	if(Njet<=1){continue;}
